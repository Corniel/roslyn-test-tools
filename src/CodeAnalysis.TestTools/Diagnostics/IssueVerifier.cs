...
public static class IssueVerifier
{
    ...
    public static string GetReportInfoHeader(this IEnumerable<Issue> issues)
    {
        ...
        static string Report(string prefix, int issues, int errors, bool force = false)
        {
            ...
            return (issues, errors, force) switch
            {
                (> 0, > 0, _) => $"* {prefix} {string.Join(", and ", components.Where(c => c[0] != '0'))}.{Environment.NewLine}",
                (_, _, true) => $"* {prefix} 0 issues, and 0 errors.{Environment.NewLine}",
                _ => string.Empty,
            };
        }
    }
